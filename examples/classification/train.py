...
def main(gpu, cfg, profile=False):
    ...
    metric = loss_landscapes.metrics.Loss(F.cross_entropy, data, target)
    ...
    # plt.show()
    # compute loss data
    loss_data_fin = loss_landscapes.random_plane(model_final, metric, 100, STEPS, normalization='filter',
                                                 deepcopy_model=True)
    fig = plt.figure()
    ste = 100
    ax = plt.axes(projection='3d')
    X = np.array([[j / ste for j in range(STEPS)] for i in range(STEPS)])
    Y = np.array([[i / ste for _ in range(STEPS)] for i in range(STEPS)])
    np.save('x', X)
    np.save('y', Y)
    np.save('loss', loss_data_fin)
